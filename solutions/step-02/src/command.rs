    ...
    pub fn describe(&self) -> String {
        match self {
            Command::Toolchain(ToolchainCommand::Java { version }) => {
                format!(
                    "internal command: select Java {}",
                    version.as_deref().unwrap_or("default")
                )
            }
            Command::Toolchain(ToolchainCommand::Node { version }) => {
                format!(
                    "internal command: select Node {}",
                    version.as_deref().unwrap_or("default")
                )
            }
            Command::Toolchain(ToolchainCommand::Maven { version }) => {
                format!(
                    "internal command: select Maven {}",
                    version.as_deref().unwrap_or("default")
                )
            }
            Command::Check(CheckKind::Lint) => "internal command: run fake lint".to_string(),
            Command::Check(CheckKind::Test) => "internal command: run fake test suite".to_string(),
            Command::Check(CheckKind::Format) => {
                "internal command: run fake format check".to_string()
            }
            Command::Verify => "internal command: verify the fake project".to_string(),
            Command::Build(BuildTarget::Java) => {
                "internal command: build fake java artifact".to_string()
            }
            Command::Build(BuildTarget::Node) => {
                "internal command: build fake node bundle".to_string()
            }
            Command::Build(BuildTarget::All) => {
                "internal command: build all fake artifacts".to_string()
            }
            Command::Image(ImageCommand::Build) => "internal command: build fake image".to_string(),
            Command::Image(ImageCommand::Publish) => {
                "internal command: publish fake image".to_string()
            }
            Command::Deploy(DeployTarget::Sandbox) => {
                "internal command: deploy fake release to sandbox".to_string()
            }
        }
    }
}
...
mod tests {
    ...
    #[test]
    fn maps_build_all() {
        let cli = Cli::parse_from(["kata-ci", "build", "all"]);

        assert_eq!(Command::from_cli(cli), Command::Build(BuildTarget::All));
    }

    #[test]
    fn maps_toolchain_without_version() {
        let cli = Cli::parse_from(["kata-ci", "java"]);
