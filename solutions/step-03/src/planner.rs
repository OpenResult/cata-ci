...
fn plan_deploy(target: &DeployTarget) -> Plan {
    match target {
        DeployTarget::Sandbox => Plan {
            ...
            summary: "Deploy the fake release to the sandbox environment".to_string(),
...
mod tests {
    ...
    #[test]
    fn verify_has_three_steps_in_a_stable_order() {
        let plan = plan(&Command::Verify);

        assert_eq!(plan.steps.len(), 3);
        assert_eq!(plan.steps[0].action.human, "run fake lint");
        assert_eq!(plan.steps[1].action.human, "run fake test suite");
        assert_eq!(plan.steps[2].action.human, "run fake format check");
    }

    #[test]
    fn build_all_contains_both_build_steps() {
    ...
    #[test]
    fn deploy_sandbox_requires_fake_env() {
        let plan = plan(&Command::Deploy(DeployTarget::Sandbox));

        assert_eq!(plan.required_env, vec!["FAKE_ENV=sandbox"]);
    }
}
